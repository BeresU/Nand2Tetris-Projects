...
def parse_lines(file_lines, symbol_table):
    ...
    for line in file_lines:
        line_results = line_parser.parse_line(line_results)

        match line_results.resultsType:
            case ParseResultsType.EMPTY:
                continue
            case ParseResultsType.CONSTANT:
                constant_to_binary = binary_converter.convert_to_binary(line_results.resultsData)
                parsed_lines.append(constant_to_binary)
            case ParseResultsType.LABEL | ParseResultsType.VARIABLE:
